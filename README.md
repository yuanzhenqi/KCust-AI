...
<p align="center">
  <strong>面向销售、商务和个人经营者的客户管理 Agent</strong>
</p>

<p align="center">
  把客户资料、沟通记录、待办提醒和 AI 对话放进同一个移动工作台，让每一次客户沟通都能沉淀成可执行的下一步。
</p>
...
KCUST AI 不是传统 CRM 表格，也不是单纯的聊天机器人。它把客户工作台、AI Agent、待办提醒、语音输入和安卓悬浮助手放在一起，让“这个客户是谁、说过什么、现在该做什么”变成一个连续闭环。
...
<table>
  <tr>
    ...
    <td width="52%">
      <h3>客户工作台叠加 Agent</h3>
      <p>Agent 不单独变成一个孤立聊天页，而是服务于客户、待办、提醒和沟通记录。用户可以在工作台里查看今日重点，在 Agent 页里处理复杂指令，也可以通过安卓悬浮窗在微信、电话记录或其他 App 上方快速录入。</p>
      <ul>
        <li>当前动作与确认卡片</li>
        <li>对话历史与处理中间态</li>
...
<table>
  <tr>
    <td width="50%">
      <h3>客户工作台优先</h3>
      <p>首屏聚焦今日待跟进、重点客户、低健康度客户和下一步建议，打开 App 就知道先处理谁。</p>
    </td>
    <td width="50%">
      <h3>一句话沉淀客户资料</h3>
      <p>从“客户微信名、所在城市、预算、需求、沟通内容、是否加急、服务价值”等自然语言中生成结构化草稿。</p>
    </td>
  </tr>
  <tr>
    <td width="50%">
  ...
  <tr>
    <td width="50%">
      <h3>自定义客户画像</h3>
      <p>画像摘要字段可以按行业调整，预算、决策人、来源渠道、核心需求、风格偏好等字段都能由用户配置。</p>
    </td>
    <td width="50%">
      <h3>模板切换不丢资料</h3>
      <p>通用销售、家装定制、商务跟进等模板只改变展示和 Agent 提取范围，历史字段仍留在字段库里。</p>
    </td>
  </tr>
</table>
...
| 客户 | 客户 CRUD、客户详情、回收站、需求标签、自定义画像摘要 |
| Agent | 意图识别、结构化草稿、客户查询、批量动作、处理中间态 |
| 待办 | 客户关联待办、到期时间、完成状态、通知与日历写入 |
| 悬浮窗 | 侧边吸附、底部按住录音、最近待办、确认卡片 |
| 设置 | 模型选择、画像字段模板、悬浮窗参数、原生能力状态、回收站 |

## 可配置客户画像

KCUST AI 的画像字段不是写死的一组 CRM 栏位。用户可以选择模板，也可以手动新增、删除、隐藏和恢复字段；Agent 会按照当前启用字段理解对话，并把提取结果写入客户档案。

```text
通用销售：预算 / 决策人 / 核心需求 / 来源渠道 / 下次跟进
家装定制：预算 / 家庭结构 / 风格偏好 / 重点空间 / 下次跟进
商务跟进：公司 / 角色 / 机会阶段 / 决策人 / 下一步
```

切换模板只会改变摘要展示和 Agent 提取字段，不会删除已有客户资料。原来的字段会保留在字段库中，可随时重新启用。
...
<table>
  ...
  <tr>
    ...
    <td>OpenAI-compatible Chat Completions, local guardrails, configurable profile extraction, structured confirmation cards</td>
  </tr>
  <tr>
...
这是一个持续迭代中的个人客户管理 Agent 原型。当前重点是打磨安卓真机体验、悬浮窗语音交互、模型 Agent 稳定性和可配置客户画像能力。
